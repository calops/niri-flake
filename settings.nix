    ...
    let
      ...
      borderish =
        ...
        ordered-section [
          ...
          {
            active =
              optional (newtype (link-type "decoration") (decoration "${path}.active")) {
            ...
            inactive =
              optional (newtype (link-type "decoration") (decoration "${path}.inactive")) {
                color = "rgb(80 80 80)";
              }
              // {
                visible = "shallow";
      ...
      border-rule =
        ...
        ordered-section [
          ...
          {
            active = nullable (newtype (link-type "decoration") (decoration "${path}.active")) // {
              visible = "shallow";
              description = ''
                The color of the ${name} for the window that has keyboard focus.
              '';
            };
            inactive = nullable (newtype (link-type "decoration") (decoration "${path}.inactive")) // {
              visible = "shallow";
              description = ''
                The color of the ${name} for windows that do not have keyboard focus.
              '';
          ...
          inherit
            (record (
              { config, ... }:
              {
                ...
                config = lib.mkMerge (map (f: f config) config');
              }
            ))
            name
            description
                      ...
                      value =
                        if def.value ? action then
                          ...
                          lib.warn
                            ...
                            ''
                            (if builtins.isString def.value then { action.${def.value} = [ ]; } else { action = def.value; });
                    }
                  ) defs
                );
            };
        ...
        layout = ordered-section [
          ...
          }
          {
            shadow = {
              ...
              softness = optional types.float 30 // {
                description = ''
                  The softness of the shadow, measured in logical pixels.
                '';
              };

              spread = optional types.float 5 // {
                description = ''
              ...
              color = optional types.str "rgba(0 0 0 50%)" // {
                description = ''
                  The color of the shadow.

                  This is a CSS color, and can be any valid CSS color string.
                '';
              };
            };
          }
          {
            insert-hint =
              ordered-section [
                ...
                {
                  display =
                    optional (newtype (link-type "decoration") (
                      decoration "programs.niri.settings.layout.insert-hint.display"
                      ...
                      lib.concatMapAttrs (
                        ...
                        lib.const {
                          ${name} = readonly (record {
                            is-defined = readonly types.bool (config.${name} != (removeAttrs defaults.${name} [ "unstable" ]));
                          }) { };
                        }
                      ) defaults
                    )) { }
  ...
  render =
    ...
    if cfg == null then
      ...
      let
        ...
        animation' =
          shader: name: cfg:
          optional-node (cfg._internal_niri_flake.${name}.is-defined || shader != null) (
            animation shader name cfg.${name}
          );
      ...
      [
        ...
        (nullable plain "switch-events" (
          ...
          in
          if lib.remove null children == [ ] then null else children
        ))

        (map workspace (
          builtins.sort (a: b: a.sort-name < b.sort-name) (
            lib.mapAttrsToList (
              key: cfg:
              cfg
              // {
                sort-name = key;
                name = if cfg.name != null then cfg.name else key;
              }
            ) cfg.workspaces
          )
        ))
